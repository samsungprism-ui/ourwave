    ...
    for subset in ["train", "test"]:
        ...
        for track_folder in sorted(tracks):
            ...
            if not os.path.exists(acc_path):
                ...
                # Ensure all stems are of the same length
                min_length = min([audio.shape[1] for audio in stem_audio])
                stem_audio = [audio[:, :min_length] for audio in stem_audio]  # Crop all to the shortest length
    ...
    for subset in ["train", "test"]:
        ...
        for track in sorted(tracks):
            ...
            if os.path.exists(mix_path):
                ...
                paths.update({key: track_path + "_" + key + ".wav" for key in ["bass", "drums", "other", "vocals", "wind"]})
            ...
            paths = dict()
            stem_audio = dict()
            for stem in ["bass", "drums", "other", "vocals", "wind"]:
            ...
            # Add other instruments to form accompaniment
            acc_audio = np.clip(sum([stem_audio[key] for key in list(stem_audio.keys()) if key not in ["vocals"]]), -1.0, 1.0)
