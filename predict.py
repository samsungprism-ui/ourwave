...
def main(args):
    ...
    # Define the output folder based on user input or default to input folder location
    output_folder = os.path.dirname(args.input) if args.output is None else args.output
    for inst in preds.keys():
        output_path = os.path.join(output_folder, os.path.basename(args.input) + "_" + inst + ".wav")
        data.utils.write_wav(output_path, preds[inst], args.sr)
        print(f"Saved {inst} to {output_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--instruments', type=str, nargs='+', default=["bass", "drums", "other", "vocals", "wind"],
                        help="List of instruments to separate (default: \"bass drums other vocals wind\")")
